    ...
    public class TrialsProfile:Profile
    {
        ...
        public TrialsProfile()
        {
            CreateMap<Trial, TrialSummaryItemModel>()
                ...
                .ForMember(dest => dest.IsExpired, options => options.MapFrom(src => src.EndDate.HasValue && src.EndDate.Value.Date < DateTime.Now.Date))
                .ForMember(dest => dest.DaysLeft, options => options.MapFrom(src => src.EndDate.HasValue ? src.EndDate.Value.Date.Subtract(DateTime.Now.Date).Days:-1));
